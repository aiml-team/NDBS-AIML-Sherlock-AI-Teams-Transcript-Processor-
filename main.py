...
async def _run_pipeline(job_id: str, file_data: list[tuple[str, bytes]],
    ...
    `prospect_name`, when supplied by the caller (the Web App), is stamped
    into the final_data dict and used as the cover-page client name in the
    rendered DOCX.
    """
    try:
        ...
        # Stamp the caller-supplied prospect_name into final_data so it
        # rides along in the /json/{id} response and is used by build_docx.
        if prospect_name:
        ...
        docx_bytes    = build_docx(final_data, template_path=template_path,
                                   prospect_name=prospect_name or final_data.get("prospect_name"))

        b64           = base64.b64encode(docx_bytes).decode()
        # Use prospect_name first, fall back to LLM-extracted client_name, else generic.
        name_for_file = (prospect_name
                         or final_data.get("prospect_name")
                         or final_data.get("client_name")
                         or "Customer_Profile")
        safe_name     = re.sub(r"[^a-zA-Z0-9_\-]", "_", name_for_file)
        filename      = f"{safe_name}_Discovery_Profile.docx"
...
async def process(
    ...
    Optional `prospect_name` form field: when supplied by the Web App
    orchestrator, it's used to fill the cover page {{ prospect_name }}
    placeholder and is included in the /json/{id} response.
    """
    if not files:
    ...
    job_id = _new_job()
    logger.info("Job %s created for %d file(s) (prospect_name=%r)",
                job_id, len(file_data), prospect_name)
